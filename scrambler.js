...
let edgeCond, cornerCond, valid, oddProb;
let evenEdgeCDF, oddEdgeCDF, evenCornerCDF, oddCornerCDF;
function getProbability(edgeText, cornerText){
    edgeCond = x => true;
    cornerCond = x => true;
    valid = true;
    ...
    if (edgeText.trim() != ''){
        try {
            const replacedInput = edgeText.replace(regex, 'x.$1');
    ...
    if (cornerText.trim() != ''){
        try {
            const replacedInput = cornerText.replace(regex, 'x.$1');
            const parsedFunc = new Function('x', `return ${replacedInput};`);
            if (typeof parsedFunc(ec_ls[0]) === 'boolean') {
                cornerCond = parsedFunc;
            }
            else {
                throw new Error('Invalid function');
            }
    ...
    valid = valid && (evenEdgeCount != 0 && evenCornerCount != 0 || oddEdgeCount != 0 && oddCornerCount != 0);
...
function getScramble(){
    ...
    return min2phase.scramble(genCube(genCode(
